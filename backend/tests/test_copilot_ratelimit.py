...
- Preserved honest fallback when ANTHROPIC_API_KEY is not configured
- Input validation
"""
...
def test_copilot_fallback_when_api_key_absent():
    """When ANTHROPIC_API_KEY is not set, endpoint returns honest guidance with 200 OK."""
    client = TestClient(app, raise_server_exceptions=False)
    with patch("backend.api.copilot.settings.anthropic_api_key", ""):
        ...
        assert (
            "ANTHROPIC_API_KEY" in data["answer"]
            or "requires an Anthropic API key" in data["answer"]
        )
